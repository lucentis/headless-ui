...
export function useTabs(props: UseTabsProps = {}): TabsApi {
    ...
    // maps each tab value to the trigger ID generated by useTabsTrigger
    const triggerIdMap = ref<Map<string, string>>(new Map())
    ...
    return {
        ...
        actions,
        bindings: {},
        registerTrigger: (tabValue: string, triggerId: string) => {
            triggerIdMap.value.set(tabValue, triggerId)
        },
        unregisterTrigger: (tabValue: string) => {
            triggerIdMap.value.delete(tabValue)
        },
        getTriggerId: (tabValue: string) => {
            return triggerIdMap.value.get(tabValue) ?? ''
        },
    }
}